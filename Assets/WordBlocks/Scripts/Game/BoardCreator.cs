...
namespace BBG.WordBlocks
{
    public class BoardCreator : MonoBehaviour
    {
        ...
        private class Board
        {
            ...
            //new added
            public int Rows { get; set; } // Make sure this property exists
            public int Columns { get; set; } // Make sure this property exists
        }
        ...
        public static BoardData Create(List<string> words, int preferredRows, int preferredCols, int seed = 0)
        {
            ...
            // If PlaceWords failed it means the preferred rows/cols given where to small to place all the words on the board
            ...
            for (int extra = 0; extra < 1000; extra++)
            {
                ...
                {
                    // Apply gravity to slide letters down after all words are placed
                    ApplyGravity(board);
            ...
            Debug.LogErrorFormat("[BoardCreator] Failed creating a board with the given input");
        ...
        /// <summary>
        /// Creates the initial board of characters
        /// </summary>
        ...
        /// <summary>
        /// Applies gravity to the board, causing letters to slide down to fill empty spaces.
        /// </summary>
        private static void ApplyGravity(Board board)
        {
            for (int x = 0; x < board.xCells; x++)
            {
                int emptyY = -1; // Track the first empty cell in the column

                for (int y = 0; y < board.yCells; y++)
                {
                    if (board.grid[x][y].letter == '\0')
                    {
                        if (emptyY == -1) emptyY = y; // Found the first empty cell
                    }
                    else if (emptyY != -1)
                    {
                        // Move the letter down to the empty space
                        board.grid[x][emptyY].letter = board.grid[x][y].letter;
                        board.grid[x][y].letter = '\0';

                        // Move the empty cell pointer down
                        emptyY++;
                    }
                }
            }

            PrintCells(board, "After applying gravity");
        }
        ...
        private static bool PlaceWords(Board board, List<string> words, int nextWordIndex)
        {
            ...
            /*           if (nextWordIndex >= words.Count)
                       ...
                       for (int i = 0; i < possiblePlacements.Count; i++)
                       {
                           ...
                           // Apply gravity after placing the word
                           ApplyGravity(board);
            ...
            for (int i = 0; i < possiblePlacements.Count; i++)
            {
                ...
                // If we get here, placing the word at the startingCell makes it so one or more of the remaining words cannot be placed,
                // so remove the word and try again with a different starting cell
                UndoChanges(board);
                PrintCells(board, "Place failed, after undo");
            }
        ...
        /// <summary>
        /// Changes the cell's letter, logs an Undo action
        /// </summary>
