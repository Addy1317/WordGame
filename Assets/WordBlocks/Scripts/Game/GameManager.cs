...
{
	public class 
	GameManager : SaveableManager<GameManager>
	{
		...
		public bool TryShowInterstitialAd(System.Action onAdFinished = null)
		{
			if (NumLevelsTillNextAd <= 0 && NextAdAt <= Utilities.SystemTimeInMilliseconds)
			{
				...
				#if BBG_MT_ADS
				adShown = BBG.MobileTools.MobileAdsManager.Instance.ShowInterstitialAd(() =>
				{
		...
		protected override void LoadSaveData(bool exists, JSONNode saveData)
		{
			...
			LastCompletedLevel			= saveData["last_level_completed"].AsInt;
			NumCoins					= saveData["num_coins"].AsInt;
			NumLevelsTillNextAd			= saveData["num_levels_till_next_ad"].AsInt;
			NumExtraWordLettersFound	= saveData["extra_letters"].AsInt;
			HintAmounts					= LoadValues(saveData["hint_amounts"].AsArray);

			JSONArray levelSaveDatasJson = saveData["level_save_data"].AsArray;
